//
//  CGXCollectionCategoryWaterLayout.m
...
@interface CGXCollectionCategoryWaterLayout ()
...
@implementation CGXCollectionCategoryWaterLayout

- (void)prepareLayout {
    [super prepareLayout];
    
    NSAssert(self.dataSource != nil, @"CGXCollectionCategoryWaterLayout.dataSource cann't be nil.");
    ...
    for (NSInteger section=0; section < numberOfSections; section++) {
        ...
        NSAssert(columnOfSection > 0, @"[CGXCollectionCategoryWaterLayout collectionView:layout:numberOfColumnInSection:] must be greater than 0.");
        ...
        if (self.dataSource && [self.dataSource respondsToSelector:@selector(collectionView:layout:referenceHeightForHeaderInSection:)]) {
            CGSize headerSize = [self.dataSource collectionView:collectionView layout:self referenceHeightForHeaderInSection:section];
        ...
        if ([self.dataSource respondsToSelector:@selector(collectionView:layout:referenceHeightForFooterInSection:)]) {
            CGSize footerSize = [self.dataSource collectionView:collectionView layout:self referenceHeightForFooterInSection:section];
            footerHeader = footerSize.height;
        }
...
- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    // Header view hover.
    if (_sectionFootersPinToVisibleBounds) {
        for (UICollectionViewLayoutAttributes *attriture in result) {
